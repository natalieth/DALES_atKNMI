...
def merge_cross(base, variable, expnr, skip, t0, dir):
    """
    ...
    itot, jtot, ktot, npx, npy = read_namelist(expnr,dir)
    xycross = ['xy','cape','surf']
    # Determine some settings
    if 'xz' in base:
        mode = 'xz'
    elif any(n in base for n in xycross):
        mode = 'xy'
    elif 'yz' in base:
        mode = 'yz'
    ...
    ni = 1 if ('yz' in base) else npx
    for i in range(ni):
        n = 1 if ('xz' in base) else npy
        for j in range(n):
            ...
            if i==0 and j==0:
                src = nc4.Dataset(dir+'/'+fname)
                dst = nc4.Dataset(dir+'/{0:}.{1:}.{2:03d}.nc'.format(base, variable, expnr), 'w')
                    ...
                    if name[0] == 'x':
                        ...
                        if name.replace(mode, '') == variable:
                            if mode == 'xy':
                                ...
                                dst.variables[name][:,:,sy] = src.variables[name][t0::skip]
        dst.sync() 
...
if __name__ == '__main__':
    ...
    p.add_argument('directory', type=str, help='Working directory of the run "/path/to/run/"')
    ...
    # Merge cross-section
    merge_cross(args.crosstype, args.crossname, args.expnr, skip, t0,args.directory)
